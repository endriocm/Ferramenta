...
const RevenueBovespa = () => {
  ...
  const [entries, setEntries] = useState(() => loadRevenueList('bovespa'))
  ...
  const handleExportTable = useCallback(async () => {
    ...
    await exportXlsx({
      fileName: `receita_bovespa_${safePeriod}_${timestamp}.xlsx`,
      sheetName: 'Bovespa',
      columns: headers,
      rows: rowsToExport,
    })
...
const handleFolderSelection = useCallback((files) => {
  ...
  const handleSync = useCallback(async (file) => {
    ...
    try {
      ...
      console.log('[IMPORT][Bovespa] rawRows=', rawRows, 'validRows=', validRows, 'savedRows=', savedRows)
      setSyncResult({
        moduleLabel: 'Bovespa',
      ...
      setLastSyncAt(now)
      try {
        localStorage.setItem('pwr.receita.bovespa.lastSyncAt', now)
      } catch {
        // noop
      }
  ...
  return (
    <div className="page">
      ...
      <section className="panel">
        ...
        <DataTable rows={pagedRows} columns={columns} emptyMessage="Sem dados Bovespa." />
