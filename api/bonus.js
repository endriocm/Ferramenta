const { getBonusResult } = require('./lib/bonus')
const { normalizeDateKey, normalizeTicker } = require('./lib/dividends')
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    try {
      const result = await getBonusResult({ ticker, from, to, includeEvents })
      res.status(200).json({
  ...
  try {
    ...
    res.status(500).json({ error: 'Falha ao processar bonificacoes.' })
  }
}
